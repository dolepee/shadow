...
EOA signatures must be canonical and non-malleable. ERC-1271 validation is supported only with a reentrancy guard around the complete call and state transition and requires the exact `0x1626ba7e` magic value. A revert, wrong magic value, short/malformed return, or reentrant attempt is invalid and leaves nonce, debt, receipts, and funds unchanged. A signature from an old line, old terms version, other chain, or other contract is invalid.
