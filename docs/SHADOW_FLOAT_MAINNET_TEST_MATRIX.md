...
| `RED-04` | `test_RED_04_feeOrTermsChangeInvalidatesPriorSignature` | `SIG-01`, `SIG-02`, `GOV-01`, `SCOPE-01` | Red against V2: exact fee and terms are not signed; Packet C passes the protection by having no fee surface and rejecting stale terms |
...
| `INV-01` | token balance covers aggregate sponsor obligations after arbitrary transitions, with no fee balance or withdrawal path | `CAP-02`, `DEBT-02`, `STATE-01`, `SCOPE-01` | Packet C invariant |
...
| `SIG-01A` | mutate agent, sponsor, line ID, line epoch, terms hash, provider, endpoint, provider principal, maximum total debt, due time, nonce, signature expiry, and executor independently | `SIG-01` | Packet C table/fuzz |
...
| `SIG-01F` | freshly signed cases enforce expiry before/at/after its boundary, nonzero executor equality, and resulting principal debt at/above maximum total debt | `SIG-01` | Packet C boundary/table/fuzz |
| `SIG-02A` | any sponsor policy or material terms change invalidates pending intent | `SIG-02`, `GOV-01` | Packet C |
| `SIG-03A` | ERC-1271 exact magic succeeds; wrong magic, short/malformed return, and validation revert fail without consuming nonce, writing payment/debt receipts, or moving funds; a guarded nested state-changing call fails without paying or mutating state, and if the wallet catches that failure then returns exact magic, the outer authorized execution completes exactly once | `SIG-01`, `SIG-03`, `CAP-03` | Packet C adversarial table |
...
| `GOV-01A` | reductions/removals immediate; cap increases queued and inactive before delay | `GOV-01` | Packet C |
| `GOV-01B` | cap activation cannot exceed immutable maxima or sponsor-accepted ceilings | `GOV-01` | Packet C fuzz |
| `GOV-01C` | every cap proposal, cancel, and activation emits exact old/new/time values | `GOV-01`, `STATE-01` | Packet C |
| `GOV-01D` | ABI, storage layout, events, and every governor/operator path expose no fee proposal, activation, accrual, charge, debt, or withdrawal mechanism | `GOV-01`, `SCOPE-01` | Packet C ABI/source/storage test |
...
| `REL-01` | deterministic manifest matches chain, token, zero-fee config, compiler, bytecode, owner, and operators | `TOK-01`, `ROLE-01`, `STATE-01`, `SCOPE-01` | Packet C release test |
| `SIZE-01` | deployed core runtime is at most 18,432 bytes | `SCOPE-01` | Packet C CI gate |
| `SCOPE-01A` | ABI/source deny fees and fee withdrawal, legacy owner credit, shared treasury, pooled capital, sponsor yield, transferable debt, leverage, token, PolicyPool, Morpho, Uniswap, style adapters, automatic scoring, and upgrade proxies | `SCOPE-01` | Packet C boundary test/review |
