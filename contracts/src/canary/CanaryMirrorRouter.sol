...
import {MirrorFeeSplitter} from "./MirrorFeeSplitter.sol";

/// @notice CANARY ONLY. A literal copy of MirrorRouter.sol with exactly one diff:
/// the inline fee-accrual in `_processFollower` is replaced with a call to
/// `MirrorFeeSplitter.settleMirrorFee`. Every other line, including all follower,
/// intent, and position logic, is byte-for-byte identical to the real MirrorRouter.
/// This file is never deployed to production Shadow infrastructure.
contract CanaryMirrorRouter {
    ...
    // sourceKickbackUSDC / protocolFeesUSDC removed: fee accrual state now lives on
    // MirrorFeeSplitter (this is the only structural change beyond the two-line diff
    // in _processFollower — claims move from this router to the splitter).
    ...
    MirrorFeeSplitter public immutable feeSplitter;

    constructor(address usdc_, address amm_, address registry_, address feeSplitter_) {
        if (usdc_ == address(0) || amm_ == address(0) || registry_ == address(0) || feeSplitter_ == address(0)) {
        ...
        feeSplitter = MirrorFeeSplitter(feeSplitter_);
        if (address(feeSplitter.usdc()) != usdc_) revert ZeroRecipient();
    }
        ...
        internal
    {
        ...
        // --- THE DIFF: replaces the two-line inline accrual with a call to the
        // preconfigured MirrorFeeSplitter. Router approves the splitter for exactly
        // mirrorFeeUSDC, then resets to 0 after — mirrors the AMM approve pattern two
        // lines below. settleMirrorFee never reverts, so this can never break the
        // follower loop.
        require(usdc.approve(address(feeSplitter), mirrorFeeUSDC), "SPLITTER_APPROVE_FAILED");
        feeSplitter.settleMirrorFee(sourceAgent, mirrorFeeUSDC);
        require(usdc.approve(address(feeSplitter), 0), "SPLITTER_APPROVE_RESET_FAILED");
    ...
    // claimSourceKickback / claimProtocolFees removed from the router: fee state and
    // claims now live on MirrorFeeSplitter (feeSplitter.claimSourceKickback /
    // feeSplitter.claimProtocolFees). This is the one other structural consequence of
    // the diff, beyond the two-line accrual swap.
}
